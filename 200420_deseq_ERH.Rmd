...
```
```{r}
dfCount <- read.table(file="count_ERH.txt", sep='\t', header=TRUE, row.names=1)
dfCount
```
```{r}
dfCount <- dfCount[,1:7]
...
```{r}
dfSample <- read.table("sampletype_ERH.txt", sep='\t', header=TRUE, row.names=1)
dfSample
```
```{r}
...
```
```{r}
write.csv(as.data.frame(resErh), file='deseq_results_ERH.csv')
```
